...
import de.paladinsinn.tp.dcis.events.MissionSaveEvent;
...
public class MissionForm extends Composite<Div> implements LocaleChangeObserver, TranslatableComponent, Serializable, AutoCloseable {
    ...
    public void init() {
        if (mission == null) {
            LOG.debug("CanÄt initialize without having data.");
            return;
        }
    ...
    public void setMission(@NotNull Mission mission) {
        if (this.mission != null && this.mission.equals(mission)) {
            LOG.info("Mission report didn't change. Ignoring event. code={}, id={}",
                    this.mission.getCode(), this.mission.getId());
    ...
    public void translate() {
        if (user == null || mission == null || locale == null) {
            LOG.warn(
        ...
        LOG.debug("Building mission edit form. locale={}, mission={}", locale, mission);
